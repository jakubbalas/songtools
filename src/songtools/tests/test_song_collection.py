...
def test_get_list_of_incorrectly_formatted_songs_in_collection(test_folder):
    ...
    make_simple_song_file(
        test_folder, "test", "artist", "Artist - Test (original mix).mp3"
    )
    make_simple_song_file(
        test_folder, "Song 1", "Other, Artist", "Other, Artist - Test.mp3"
    )
    ...
    with patch(
        "songtools.song_collection.config.collection_path", test_folder.absolute()
    ):
...
def test_syncing_collection_makes_db_consistent_with_folder(test_folder):
    ...
    dance_folder.mkdir()
    make_simple_song_file(
        dance_folder, "song 1", "dance artist", "Dance Artist - Test.mp3"
    )
    ...
    with Session(db_engine) as session:
        session.add_all(
            [
                CollectionSong(
                    name_hash="tobedeleted",
                    file_path="Dance/JDP - NotInteresting.mp3",
                    file_size=1234,
                ),
                HeardSong(
                    name_hash="toberemoved",
                    file_name="Dance/JDP - Kicked Out.mp3",
                    in_collection=True,
                ),
                HeardSong(
                    name_hash=song_dance_2_file.name_hash,
                    file_name="Dance/Dance Artist - Test.mp3",
                    in_collection=False,
                ),
                HeardSong(
                    name_hash=song_rock_2_hash.name_hash,
                    file_name="Rock/Rock Artist - Test.mp3",
                    in_collection=False,
                ),
            ]
        )
    ...
    with Session(db_engine) as session:
        ...
        heard_songs = session.scalars(
            select(HeardSong).where(HeardSong.in_collection == True)  # noqa
        ).all()
